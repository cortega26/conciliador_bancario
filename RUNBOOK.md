<!--
Elegimos el nombre RUNBOOK.md porque este documento esta orientado a operacion repetible (paso a paso),
resolucion de problemas y referencia permanente para uso mensual/periodico.
-->
...
`conciliador-bancario` es una herramienta **local** (CLI) para conciliar:
- Movimientos del banco (cartola/extracto) contra
- Movimientos internos “esperados” (ERP, planilla interna, registros contables)

Entrega resultados **auditables** y **conservadores**:
- Si hay ambigüedad, **no concilia** automáticamente (fail-closed).
- No hay “magia” silenciosa: errores y hallazgos son explícitos.
- Misma entrada produce el mismo `run.json` (sin timestamps variables).
...
- Auditoría interna: conservar evidencia técnica (`run.json`, `audit.jsonl`, XLSX técnico).
...
OCR (solo si necesita procesar PDF escaneado):
...
- PDF (`.pdf`) con texto extraíble (cartola digital)
- PDF escaneado: solo si habilita OCR (ver sección 3.3)

#### Campos obligatorios (mínimos)
Debe existir informacion equivalente a:
- Fecha de operación
- Monto
...
Referencia:
- Se normaliza (se eliminan espacios y se pasa a mayuscula). Esto afecta matches por referencia.

#### Ejemplo valido (Banco CSV)
```csv
...
El sistema primero intenta leer texto del PDF. Si el PDF parece escaneado (sin texto extraible):
...
Si no esta seguro, use CSV o XLSX.
...
- `umbral_autoconcilia`: umbral de score para autoconciliar.
- `umbral_confianza_campos`: umbral de confianza por campo para permitir autoconciliacion.
...
Si falla, el comando muestra errores concretos (por ejemplo: columnas faltantes, fecha invalida, monto invalido).

### Paso 3: Ejecutar en modo seguro (dry-run)
...
- Incluye `fingerprint` (hashes de inputs y config) para trazabilidad.
...
## 7. Opciones de Conciliacion (como funciona a alto nivel)
...
- `rechazado`: reservado para casos donde el motor marque explicitamente que no corresponde (segun reglas internas).

### Trade-offs principales
- Subir `ventana_dias_monto_fecha` puede aumentar sugerencias, pero también riesgo de falsos positivos.
- Bajar `umbral_autoconcilia` aumenta autoconciliacion, pero incrementa riesgo.
- Habilitar OCR permite procesar PDFs escaneados, pero baja confianza y bloquea autoconciliacion.

### Masking y privacidad
Por defecto `--mask` esta activo en `run`. Puede desactivarse con `--no-mask` (no recomendado).
El masking:
- Enmascara patrones sensibles en outputs (cuentas largas, RUT).
- Aplica proteccion basica contra “Excel/CSV injection” en el XLSX (celdas que comienzan con `=`, `+`, `-`, `@`).

## 8. Modelo Freemium vs Premium (resumen operativo)

Freemium/Core (este repo):
- Genera `run_dir` con `run.json`, `audit.jsonl` y (opcional) `reporte_conciliacion.xlsx`.
...
- Consume el `run_dir` del core y genera salidas adicionales enfocadas en productividad:
  - `review`: priorizacion y agrupacion de pendientes.
...
### “Fila X: fecha ... invalida”
Causa:
- Fecha con formato no soportado.
Solución:
- Normalice a `dd/mm/aaaa` o `aaaa-mm-dd`.

### “Fila X: monto ... invalido”
Causa:
- Monto contiene texto o separadores no compatibles.
Solución:
- Use numeros y separadores comunes (ej: `150.000`, `-250000`, `$ 1.234.567`).

### “XLSX: no se encontro una hoja con las columnas requeridas”
Causa:
...
### “PDF parece escaneado ... OCR esta deshabilitado”
Causa:
- El PDF no tiene texto extraible.
Solución:
...
## 10. Buenas Practicas (para minimizar errores)
...
- Use `--dry-run` para probar ingestion/matching antes de generar el XLSX.
- No reutilice el mismo `--out` sin limpiar `audit.jsonl` (evita mezclar auditorias).

Seguridad:
- Mantenga `--mask` activado por defecto.
- No comparta `run_dir` sin revisar si contiene datos sensibles (aunque esten enmascarados, puede haber metadata).
...
Probablemente es un escaneo sin texto. Debe habilitar OCR (opcional) y aun así se bloquea autoconciliación por baja confianza.
...
Si con `--no-mask`, pero no es recomendado por seguridad. Ademas cambia el `run_id` (porque forma parte del fingerprint).
...
- `audit.jsonl` (traza)
- `reporte_conciliacion.xlsx` (si se genero)
